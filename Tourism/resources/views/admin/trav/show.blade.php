            ...
            <div class="result_content">
                <table class="list_tab">
                    <tr>
                        ...
                        <tr>
                            <td class="tc tr_id">{{$data->tr_id}}</td>
                            <td class="tc">{{$data->us_name}}</td>
                            <td class="tc">{!! $data->tr_content !!}</td>
                            <td class="tc">
                                @foreach($pic as $p)
                                    @if($data->tr_id==$p->tr_id)
                                        <img class="imgto" width="45px" height="35px" style="border-radius: 5px;vertical-align:middle;" src="{{url($p->tp_picture)}}">
                                    @endif
                                @endforeach
                            </td>
                            <td class="tc">{{date('Y-m-d H:i:s',$data->tr_time)}}</td>
                            <td class="tc">{{$data->sc_name}}</td>
                            <td class="tc" style="text-align:center;">
                                <?php $i = 0; ?>
                                @foreach($replay as $rp)
                                    @if($rp->tr_id == $data->tr_id)
                                        <?php $i++; ?>
                                    @endif
                                @endforeach
                                <a href="{{url('admin/replay/'.$data->tr_id)}}" style="width: 100%">{{$i}}</a>
                            </td>
                            <td class="tc"  style="text-align:center;">
                                @if($data->tr_status==1)
                                    <a href="javascript:;" onclick="onStatus('{{$data->tr_id}}',0)" style="width: 100%">显示</a>
                                @else
                                    <a href="javascript:;" onclick="onStatus('{{$data->tr_id}}',1)" style="width: 100%">关闭</a>
                                @endif
                            </td>
                            <td>
                                <a href="{{url('admin/replay/create/'.'?tr_id='.$data->tr_id)}}">回复</a>
                                <a href="javascript:;" onclick="delAre('{{$data->tr_id}}')">删除</a>
                            </td>
                        </tr>
                </table>
